...
export default async function EditEventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [eventResult, bandsResult, sponsorsResult, lineupResult, eventSponsorsResult] = await Promise.all([
    ...
    supabase!.from("event_sponsors").select("sponsor_id").eq("event_id", id),
  ]);
  ...
  const selectedSponsors = eventSponsorsResult.data?.map((item) => item.sponsor_id) || [];
  ...
  return (
    <>
      ...
      <EventForm event={eventResult.data} bands={bandsResult.data || []} sponsors={sponsorsResult.data || []} selectedHeadlinerId={headliner} selectedSupportIds={supports} selectedSponsorIds={selectedSponsors} />
